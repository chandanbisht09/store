...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <>
      <motion.div
        layout
        className="group relative bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-850 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full"
      >
        {/* Wishlist Button */}
        <button
          onClick={(e) => {
        ...
        <div className="absolute top-4 left-4 z-10 flex flex-col gap-1.5 pointer-events-none">
          {product.isBestSeller && (
            <span className="px-3 py-1 text-[10px] uppercase tracking-wider font-semibold text-white bg-maroon-600 rounded-full shadow-sm">
              Best Seller
            </span>
          )}
          {product.isTrending && (
            <span className="px-3 py-1 text-[10px] uppercase tracking-wider font-semibold text-white bg-royal-500 rounded-full shadow-sm">
              Trending
            </span>
          )}
          {hasDiscount && (
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="mt-auto pt-3 border-t border-stone-50 dark:border-stone-850 flex items-center justify-between">
            <div className="flex flex-col">
              ...
              <span className="text-lg font-bold text-stone-900 dark:text-gold-100">
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-lg bg-gold-500 hover:bg-gold-600 text-stone-950 font-medium text-xs flex items-center gap-1.5 transition-all cursor-pointer active:scale-95 hover:shadow-md"
            >
              ...
              <div className="grid md:grid-cols-2">
                ...
                <div className="p-8 flex flex-col justify-between">
                  <div>
                    ...
                    <h2 className="font-serif text-2xl md:text-3xl font-medium text-stone-900 dark:text-stone-100 mb-3">
                      {product.name}
                    </h2>
                    ...
                    <div className="mb-5 flex items-baseline gap-2.5">
                      <span className="text-3xl font-bold text-stone-900 dark:text-gold-100">
                        ₹{displayPrice.toLocaleString('en-IN')}
                      </span>
                      {hasDiscount && (
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      className="flex-1 py-3.5 rounded-xl bg-gold-500 hover:bg-gold-600 text-stone-950 font-semibold text-sm flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md hover:shadow-lg hover:scale-[1.01] active:scale-95"
                    >
                      <ShoppingBag className="w-5 h-5" />
                      Add to Basket
                    </button>
