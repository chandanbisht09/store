  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 font-sans text-stone-850 dark:text-stone-100">
      ...
      <div className="flex items-center justify-between mb-8">
        <Link to="/cart" className="flex items-center gap-2 text-stone-500 hover:text-[#4B0011] dark:hover:text-gold-400 text-xs font-bold uppercase tracking-widest transition-colors">
          <ArrowLeft className="w-4 h-4" />
          Back to Artisan Basket
        </Link>
        <span className="text-[11px] font-bold text-[#D4AF37] tracking-[0.2em] uppercase">Secure Verification</span>
      </div>

      <h1 className="font-serif text-3xl font-bold text-[#4B0011] dark:text-white mb-2 leading-tight">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-7 space-y-6">
          ...
          {/* Shipping details */}
          <div className="bg-white dark:bg-stone-900 border border-[#D4AF37]/20 rounded-2xl p-6 sm:p-8 shadow-sm">
          ...
          <div className="bg-white dark:bg-stone-900 border border-[#D4AF37]/20 rounded-2xl p-6 sm:p-8 shadow-sm space-y-4">
            ...
            {/* Gift Wrap */}
            <div className="p-4 border border-[#D4AF37]/15 rounded-xl bg-stone-50/50 dark:bg-stone-950/40 flex items-start gap-3">
              <input
                type="checkbox"
                id="giftWrap"
                name="giftWrap"
                ...
                className="mt-1 h-4 w-4 border-[#D4AF37] text-[#4B0011] focus:ring-[#4B0011] rounded cursor-pointer"
              />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <button
                ...
                className={`p-4 rounded-xl border flex flex-col text-left transition-all cursor-pointer ${
                  formState.deliveryMethod === 'royal'
                    ? 'border-[#D4AF37] bg-[#4B0011]/5 dark:bg-gold-550/5'
              ...
              >
                <span className="font-serif font-bold text-[#4B0011] dark:text-white flex items-center justify-between w-full">
                  ...
                  <span className="font-sans font-normal text-xs text-[#D4AF37]">₹150</span>
                </span>
              ...
              >
                <span className="font-serif font-bold text-[#4B0011] dark:text-white flex items-center justify-between w-full">
                  ...
                  <span className="font-sans font-normal text-xs text-[#D4AF37]">₹350</span>
                </span>
          ...
          <div className="bg-white dark:bg-stone-900 border border-[#D4AF37]/20 rounded-2xl p-6 sm:p-8 shadow-sm">
            <h2 className="font-serif text-lg font-bold text-[#4B0011] dark:text-gold-200 mb-6 flex items-center gap-2">
              ...
              Sovereign Sovereign Payment Registry
            </h2>
            ...
            <div className="mt-6 p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20 flex gap-3 text-xs leading-relaxed text-emerald-800 dark:text-emerald-430 select-none">
              <ShieldCheck className="w-5 h-5 text-emerald-600 flex-shrink-0 mt-0.5" />
              <div>
                <p className="font-bold">Artisan Bank Safe Shield Activated</p>
          ...
          <button
            ...
            className="w-full py-4 rounded-xl bg-[#4B0011] hover:bg-[#D4AF37] text-white hover:text-[#4B0011] font-bold text-sm uppercase tracking-widest cursor-pointer shadow-lg hover:shadow-xl transition-all border border-[#D4AF37]/30 flex items-center justify-center gap-2"
          >
            {isSubmitting ? (
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white dark:bg-stone-900 border border-[#D4AF37]/20 rounded-2xl p-6 sm:p-8 shadow-sm h-fit">
                ...
                {cart.map((item) => {
                  ...
                  return (
                    <div key={item.product.id} className="py-3 flex gap-3.5 text-xs">
                      ...
                      <div className="text-right font-bold text-[#4B0011] dark:text-gold-200">
            ...
            {/* Price breakdown */}
            <div className="border-t border-[#D4AF37]/20 pt-4 space-y-3.5 text-xs font-semibold text-stone-500">
              <div className="flex justify-between">
                <span>Handcrafted Subtotal</span>
                <span className="text-[#2D2926] dark:text-stone-100">₹{cartSubtotal.toLocaleString('en-IN')}</span>
              </div>
              <div className="flex justify-between">
                <span>Sovereign Logistics Insurance</span>
                <span className="text-[#2D2926] dark:text-stone-100">₹{deliveryCost.toLocaleString('en-IN')}</span>
              </div>
              {formState.giftWrap && (
                <div className="flex justify-between">
                  <span>Velvet Gift Box wrap</span>
                  <span className="text-[#2D2926] dark:text-stone-100">₹250</span>
                </div>
              )}
              <div className="border-t border-[#D4AF37]/25 pt-4 flex justify-between font-bold text-sm">
                ...
                <span className="text-[#4B0011] dark:text-gold-200 text-lg">₹{cartTotal.toLocaleString('en-IN')}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {isSuccessOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="mx-auto h-16 w-16 bg-[#4B0011]/5 rounded-full flex items-center justify-center border border-[#D4AF37]/35 mb-6">
                <CheckCircle className="w-10 h-10 text-[#D4AF37] animate-bounce" />
              </div>

              <h3 className="font-serif text-2xl font-extrabold text-[#4B0011] dark:text-gold-200 mb-2">Order Authorized!</h3>
              <p className="text-xs text-[#D4AF37] uppercase tracking-[0.2em] font-extrabold mb-4">Chronicle Registered</p>
              
              <div className="p-4 bg-stone-50 dark:bg-stone-950/45 rounded-2xl border border-stone-150 dark:border-stone-850 text-xs space-y-1.5 font-semibold text-stone-500 mb-6">
                <div className="flex justify-between">
                  <span>Ledger Invoice ID</span>
                  <span className="font-mono text-[#4B0011] dark:text-gold-200 font-bold">{generatedOrderId}</span>
                </div>
                <div className="flex justify-between">
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-[#4B0011] hover:bg-[#D4AF37] text-white hover:text-[#4B0011] font-bold text-xs uppercase tracking-wider transition-all cursor-pointer border border-[#D4AF37]/20"
              >
