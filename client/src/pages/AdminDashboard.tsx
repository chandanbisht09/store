  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-[#D4AF37]/25 pb-5 h-fit">
        ...
        <div className="flex items-center gap-2 bg-[#D4AF37]/10 border border-[#D4AF37]/35 p-2.5 rounded-lg text-[#4B0011] dark:text-emerald-200 text-xs font-semibold">
          <TrendingUp className="w-4 h-4 mr-0.5 text-[#4B0011]" />
          <span>General Growth Indicators: <strong className="text-[#4B0011] font-bold">+14.2%</strong> vs May</span>
        </div>
      </div>

      {/* Grid Summary Cards (Bento style metrics) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        
        {/* Metric 1: Total Revenue */}
        <div className="bg-white dark:bg-stone-900 p-5 rounded-2xl border border-[#D4AF37]/20 shadow-sm flex items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="text-[10px] text-[#4B0011]/60 dark:text-stone-400 uppercase font-bold tracking-wider block">Total Turnover</span>
          ...
          <div className="p-4 bg-[#D4AF37]/10 text-[#4B0011] rounded-xl font-bold">
            🏛️
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-stone-900 p-5 rounded-2xl border border-[#D4AF37]/20 shadow-sm flex items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="text-[10px] text-[#4B0011]/60 dark:text-stone-400 uppercase font-bold tracking-wider block">Order volume</span>
            <span className="text-2xl font-extrabold text-[#2D2926] dark:text-stone-100 block">
              {summaryMetrics.totalOrders}
            </span>
            <span className="text-[9px] text-[#4B0011]/40 block font-medium">Insured shipments</span>
          </div>
          <div className="p-4 bg-[#D4AF37]/10 text-[#4B0011] rounded-xl font-bold">
            📦
          </div>
        ...
        <div className="bg-white dark:bg-stone-900 p-5 rounded-2xl border border-[#D4AF37]/20 shadow-sm flex items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="text-[10px] text-[#4B0011]/60 dark:text-stone-400 uppercase font-bold tracking-wider block">Registered Patrons</span>
            <span className="text-2xl font-extrabold text-[#2D2926] dark:text-stone-100 block">
        ...
        <div className="bg-white dark:bg-stone-900 p-5 rounded-2xl border border-[#D4AF37]/20 shadow-sm flex items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="text-[10px] text-[#4B0011]/60 dark:text-stone-400 uppercase font-bold tracking-wider block">Inventory Sku</span>
            <span className="text-2xl font-extrabold text-[#2D2926] dark:text-stone-100 block">
              {summaryMetrics.totalProducts}
            </span>
            <span className="text-[9px] text-[#4B0011]/40 block font-medium">4 active lines</span>
          </div>
          <div className="p-4 bg-[#D4AF37]/10 text-[#4B0011] rounded-xl font-bold">
            🗃️
          </div>
      ...
      {/* Low Stock Warning Banner (if any products inventory <= 5) */}
      {summaryMetrics.lowStockItems.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-955 dark:bg-amber-950/20 border border-amber-250 border-amber-200 p-4 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs font-medium text-amber-850 dark:text-amber-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-stone-900 shadow-sm border border-[#D4AF37]/20 p-6 rounded-2xl flex flex-col justify-between">
          <h3 className="font-serif font-bold text-[#4B0011] dark:text-stone-100 text-base mb-6 border-b border-[#D4AF37]/15 pb-3">
        ...
        <div className="bg-white dark:bg-stone-900 shadow-sm border border-[#D4AF37]/20 p-6 rounded-2xl flex flex-col justify-between">
          <h3 className="font-serif font-bold text-[#4B0011] dark:text-stone-101 text-base mb-6 border-b border-[#D4AF37]/15 pb-3">
      ...
      <div className="bg-white dark:bg-stone-900 border border-[#D4AF37]/20 p-6 rounded-2xl shadow-sm">
        <div className="flex items-center justify-between pb-3 border-b border-[#D4AF37]/15 mb-6">
          <h3 className="font-serif font-bold text-[#4B0011] dark:text-stone-100 text-base">
            Recent Transmissions Logs
          </h3>
          <Link
            to="/admin/orders"
            className="text-xs font-bold text-[#4B0011] hover:text-[#D4AF37] hover:underline uppercase tracking-wider flex items-center gap-1"
          >
            Review all sales
            ...
            <table className="w-full text-left font-sans">
              <thead>
                <tr className="border-b border-stone-100 dark:border-stone-800 text-stone-400 uppercase tracking-widest text-[9px] font-bold">
                  ...
                  <th className="py-2">Customer Recipient</th>
                  <th className="py-2">Settled Sum</th>
                  <th className="py-2">Status Flag</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-stone-100 dark:divide-stone-850/40 text-stone-650 dark:text-stone-300">
                {recentOrders.map((o) => (
                  <tr key={o.id} className="hover:bg-stone-50/60 dark:hover:bg-stone-950/20 transition-colors">
                    <td className="py-3 px-1 font-mono font-bold text-[#4B0011] dark:text-gold-102">
                      {o.orderNumber}
                    </td>
                    ...
                    <td className="py-3 px-1 font-bold text-[#4B0011] dark:text-gold-200">
                    ...
                    <td className="py-3 px-1">
                      <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase ${
                        o.status === 'Completed'
                          ...
                          : o.status === 'Shipped'
                          ? 'bg-sky-100 text-sky-850 bg-sky-102'
